...
class Command(BaseCommand):
    help = 'Denormalizes Tokens to NormTokens'
    ...
    def handle(self, *args, **options):
        errors = []
        start = options['start-end'][0]
        end = options['start-end'][1]
        starttime = datetime.now()
        self.stdout.write(
            "start with token at: {} at: {}".format(start, starttime.strftime('%Y-%m-%d %H:%M:%S'))
        )

        tokens = Token.objects.exclude(cluster__isnull=True)
        for x in tokens[start:end]:
        ...
        self.stdout.write("end with token at: {} at: {}".format(
            end, endtime.strftime('%Y-%m-%d %H:%M:%S'))
        )
        self.stdout.write('---------------')
        self.stdout.write("duration: {}".format(endtime-starttime))
